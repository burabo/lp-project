...
#include "LP_Utils.h"
    ...
    void readBool(bool *const value, char const* const message){
        ...
        while (temp < minValue || temp > maxValue) {
            printf(MSG_LEITURA_BOOLEANO_INVALIDO, minValue, maxValue);
            displayMessage(message);
            readShortFromStdin(&temp);
        }
